...
bosses = list(map(lambda row: Boss(**row),
              import_bosses_from_csv(sys.argv[4])))
base_teams = list(map(lambda row: Team(**row),
                  import_base_teams_from_csv(sys.argv[5])))

players = PlayersBuilder(
    player_stats, player_availabilities, player_experiences).build()
...
for team in base_teams:
    print(f"=== {team.boss_name} Team at {team.time}, filled {
          len(team.players)}/{team.boss.capacity}")
